...
def cmd_simulate(strat: Strategy, n: int = 252):
    ...
    print(f"=== INTEGRITY SIMULATION — last {len(df)} cached days "
    ...
    for lbl, col in [("GATE long/flat", "gate"), ("buy&hold", "bh")]:
        ...
        print(f"  {lbl:<16} ann-Sharpe={sh:>5.2f}  CAGR={cg*100:>6.1f}%  maxDD={dd*100:>6.1f}%  "
...
def cmd_open(strat: Strategy):
    ...
    print(f"OPENED forward decision {entry['asof']}: {entry['state']} "
...
def cmd_mark(strat: Strategy):
    ...
    for e in ledger:
        ...
        track.append({"asof": e["asof"], "state": e["state"], "pos": pos,
                      "gate": gate_r, "bh": bh_r, "marked_against": against})
        e["marked"] = True
        new += 1
        print(f"  MARKED {e['asof']} (vs {against}): gate {gate_r*100:+.3f}%  "
...
def cmd_status():
    if LEDGER.exists():
        ...
        for e in ledger[-8:]:
            print(f"  {e['asof']}  {e['state']:<4} ratio_ma3={e['ratio_ma3']}  marked={e['marked']}")
    else:
        print("no ledger yet.")
    if TRACK.exists():
        ...
        if len(df) >= 5:
            for lbl, col in [("GATE", "gate"), ("buy&hold", "bh")]:
                ...
                print(f"  {lbl:<10} ann-Sharpe={sh:5.2f}  CAGR={cg*100:6.1f}%  "
